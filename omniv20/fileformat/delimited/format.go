...
type delimitedFileFormat struct {
	schemaName string
}
...
func NewDelimitedFileFormat(schemaName string) fileformat.FileFormat {
	return &delimitedFileFormat{schemaName: schemaName}
}

type delimitedFormatRuntime struct {
	Decl  *fileDecl `json:"file_declaration"`
	XPath string
}

func (f *delimitedFileFormat) ValidateSchema(
	...
	var runtime delimitedFormatRuntime
...
func (f *delimitedFileFormat) validateFileDecl(decl *fileDecl) error {
...
func (f *delimitedFileFormat) validateColumns(columns []column) error {
...
func (f *delimitedFileFormat) CreateFormatReader(
	name string, r io.Reader, runtime interface{}) (fileformat.FormatReader, error) {
	rt := runtime.(*delimitedFormatRuntime)
	return NewReader(name, r, rt.Decl, rt.XPath)
}

func (f *delimitedFileFormat) FmtErr(format string, args ...interface{}) error {
