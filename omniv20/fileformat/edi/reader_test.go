...
func TestRawSegToNode(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{
			name: "fixed_length_in_bytes not matched",
			rawSeg: rawSeg{
				valid: true,
				name:  "ISA",
			...
			decl: &segDecl{
				FixedLengthInBytes: testlib.IntPtr(12),
				fqdn:               "ISA",
			},
			err:      `input 'test' between character [10,20]: segment 'ISA' expected length 12 byte(s), but got 14 byte(s)`,
			expected: "",
		},
		{
